...
OUTPUT_FILE = 'sensitivity_analysis_ES.png'
TARGET_COL = 'Actual'
...
def main():
    ...
    pattern = r"ensemble_selection_(\d+)_weeks(_weighted|_exponential)?"
    ...
    # 1. Globální minimum / Sweet Spot (6 weeks)
    try:
    ...
    # Zvýraznění oblasti stability (6-10 týdnů) - ponecháno pro kontext
    plt.axvspan(6, 8, color='yellow', alpha=0.1, label='Stable Region (6-10 weeks)')

    # Úpravy grafu
    plt.title('Sensitivity Analysis: Effect of Memory Length on Forecast Accuracy (Ensemble Selection)', fontsize=16, pad=15)
    ...
    if len(handles) > 3:
        ...
        plt.legend(title='Weighting Scheme', fontsize=11, title_fontsize=12, loc='upper right')
